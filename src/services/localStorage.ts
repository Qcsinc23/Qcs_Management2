...
export const saveGuestBooking = (booking: Omit<GuestBooking, 'timestamp'>) => {
  try {
...
export const getGuestBooking = (): GuestBooking | null => {
  try {
...
export const clearGuestBooking = () => {
  try {
    localStorage.removeItem(`${STORAGE_PREFIX}booking`);
    return true;
  } catch (error) {
...
export const saveGuestProgress = (currentStep: number) => {
  try {
    localStorage.setItem(
...
export const getGuestProgress = (): number | null => {
  try {
